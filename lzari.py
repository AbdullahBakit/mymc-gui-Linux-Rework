...
a two level dictionary look up during compression rather than
...
_tr_rev_16 = bytes.maketrans(b"\x00\x01\x02\x03\x10\x11\x12\x13\x20\x21\x22\x23\x30\x31\x32\x33",
                  b"0123456789abcdef")
...
class lzari_codec(object):
    ...
    def add_suffix_2(self, pos, find):
        ...
        if p >= 0:
            ...
            if count == 0:
                ...
                key2 = src[p2 : p2 + chars]
                if key2 in table2 and table2[key2] == p:
                    del table2[key2]
                a[0] = count
        assert (mpos == None
    ...
    def _add_suffix(self, pos, find):
        ...
        if find and r[0] != None:
            print("%4d %02x %4d %2d" % (pos - start_pos, self.src[pos], r[0] - start_pos, r[1]))
        else:
            print("%4d %02x" % (pos - start_pos, self.src[pos]))
        return r
    ...
    def encode_char(self, char):
        ...
        high = low + range_val * sym_cum[symbol - 1] // sym_cum[0]
        low += range_val * sym_cum[symbol] // sym_cum[0]
        debug(high, "high")
        debug(low, "low")
        while True:
            if high <= QUADRANT2:
    ...
    def encode_position(self, position):
        ...
        range_val = high - low
        high = low + range_val * position_cum[position] // position_cum[0]
        low += range_val * position_cum[position + 1] // position_cum[0]
        ...
        while in_pos < in_length:
            if progress:
                ...
                if percent != last_percent:
                    sys.stderr.write("%s%3d%%\r" % (progress, percent))
...
if mymcsup == None:
    ...
    def decode(src, out_length, progress = None):
        ...
        if (mylzari_decode(src, len(src), out, out_length, progress) == -1):
            raise ValueError("compressed input is corrupt")
        return ctypes.string_at(out, out_length)

    def encode(src, progress = None):
        (r, compressed, comp_len) = mylzari_encode(src, len(src), progress)
        if r == -1:
...
def main2(args):
    ...
    if args[1] == "c":
        ...
        dest = lzari.decode(src[4:], struct.unpack("<L", src[:4])[0])
        now = os.times()
    out.write(dest)
    out.close()
...
def main(args):
    if len(args) < 4:
        print("Usage: python lzari.py [c|d] input output")
        return
    ...
    if args[1] == "pc":
        ...
        import cProfile
        ret = 0
        cProfile.runctx("ret = main2(args[1:])", globals(), locals())
        return ret
