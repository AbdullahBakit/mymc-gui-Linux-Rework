...
# Windows encoding wrappers have been stripped. Linux relies on standard UTF-8.

if os.name in ["nt", "os2", "ce"]:
    from glob import glob
else:
    # assume globing is done by the shell on Linux
...
def _print_bin(base, s):
    for off in range(0, len(s), 16):
        ...
        a = s[off : off + 16]
        for b in a:
            print("%02X" % (b if type(b) is int else ord(b)), end=" ")
