...
# Bypassing round.py cache completely
def div_round_up(a, b): return int((a + b - 1) // b)
def round_up(a, b): return int((a + b - 1) // b * b)
def round_down(a, b): return int(a // b * b)
...
def pathname_split(pathname):
    ...
    return ([name for name in components if name != ""],
        components[0] != "",
        components[-1] == "")
...
class ps2mc_file(object):
    ...
    def write_file_cluster(self, n, buf):
        ...
        if (cluster < file_cluster_end or len(self.fat_chain) != file_cluster_end):
            raise corrupt("file length doesn't match cluster chain length", mc.f)
...
class ps2mc_directory(object):
    def __init__(self, mc, dirloc, first_cluster, length, mode = "rb", name = None):
        self.f = ps2mc_file(mc, dirloc, first_cluster, length * PS2MC_DIRENT_LENGTH, mode, name)
...
class ps2mc(object):
    ...
    def __init__(self, f, ignore_ecc = False, params = None):
        ...
        d8 = dot[8].decode('utf-8', 'ignore') if isinstance(dot[8], bytes) else str(dot[8])
        dd8 = dotdot[8].decode('utf-8', 'ignore') if isinstance(dotdot[8], bytes) else str(dotdot[8])
        
        if (d8 != ".") or (dd8 != "..") or not mode_is_dir(dot[0]) or not mode_is_dir(dotdot[0]):
            raise corrupt("Root directory damaged.", f)
    ...
    def write_superblock(self):
        ...
        self.f.seek(self.good_block2 * self.pages_per_erase_block * self.raw_page_size)
    ...
    def format(self, params):
        ...
        if pages_per_erase_block < 1:
            raise error("invalid pages per erase block (%d)" % page_size)
        ...
        self.version = b"1.2.0.0\x00\x00\x00\x00\x00"
        ...
        for i in range(indirect_fat_clusters):
            ...
            buf = array.array('I', range(base, base + epc))
            if (i == indirect_fat_clusters - 1 and remainder != 0):
                buf = buf[:remainder]
                buf.extend([0xFFFFFFFF] * (epc - remainder))
    ...
    def write_page(self, n, buf):
        ...
        if len(buf) != self.page_size:
            raise error("internal error: write_page: %d != %d" % (len(buf), self.page_size))
        f.write(buf)
        if self.spare_size != 0:
            a = b"".join(bytes(s) for s in ecc_calculate_page(buf))
            f.write(a)
    ...
    def read_cluster(self, n):
        ...
        return b"".join(map(self.read_page, range(n, n + pages_per_cluster)))
    ...
    def write_cluster(self, n, buf):
        ...
        if self.spare_size == 0:
            ...
            if len(buf) != cluster_size:
                raise error("internal error: write_cluster: %d != %d" % (len(buf), cluster_size))
    ...
    def _dirloc_to_ent(self, dirloc):
        dir = self._directory(None, dirloc[0], dirloc[1] + 1, name = "_dirloc_to_ent temp")
        ent = dir[dirloc[1]]
        dir.close()
        return ent
    ...
    def update_dirent_all(self, dirloc, thisf, new_ent):
        ...
        if changed:
            dir.write_raw_ent(dirloc[1], ent, (modified and not is_dir))
    ...
    def update_dirent(self, dirloc, thisf, first_cluster, length, modified):
        ...
        self.update_dirent_all(dirloc, thisf,
                    ...
                    first_cluster, None, modified, None, None))
    ...
    def search_directory(self, dir, name):
        ...
        for i in list(range(start, len(dir))) + list(range(0, start)):
            ...
            n = ent[8].decode('utf-8', 'ignore') if isinstance(ent[8], bytes) else str(ent[8])
            n2 = name.decode('utf-8', 'ignore') if isinstance(name, bytes) else str(name)
            
            if n == n2 and (ent[0] & DF_EXISTS):
                return (i, ent)
        return (None, None)
    ...
    def create_dir_entry(self, parent_dirloc, name, mode):
        ...
        dir = self._directory(parent_dirloc, dir_ent[4], dir_ent[2], "r+b")
        l = len(dir)
        assert l >= 2
        for i in range(l):
            ent = dir[i]
        ...
        ent[6] = now
        ent[7] = 0
        ent[8] = name[:32] if isinstance(name, str) else name[:32].decode('utf-8')
    ...
    def path_search(self, pathname):
        ...
        if dirloc == (0, 0):
            ...
            dir_cluster = 0
            dir = _directory(dirloc, dir_cluster, ent[2], name = tmpname)
        else:
            ent = self._dirloc_to_ent(dirloc)
            dir = _directory(dirloc, ent[4], ent[2], name = tmpname)
    ...
    def _is_empty(self, dirloc, ent, filename):
        dir = self._directory(dirloc, ent[4], ent[2], "rb", filename)
        try:
    ...
    def remove(self, filename):
        ...
        if is_dir:
            if ent[4] == 0:
                raise io_error((EACCES, "cannot remove root directory"))
    ...
    def rename(self, oldpathname, newpathname):
        ...
        if olddirloc in self.open_files:
            raise io_error((EBUSY, "cannot rename open file", newpathname))
        ...
        if oldparentdirloc == newparentdirloc:
            ...
            try:
                dir[olddirloc[1]] = (None, None, None, None,
                             None, None, None, None, newname)
            finally:
                dir.close()
            return

        if is_dir and self.is_ancestor(newparentdirloc, olddirloc):
            raise io_error((EINVAL, "cannot move directory beneath itself", oldpathname))
        ...
        try:
            ...
            (newdirloc, newent) = self.create_dir_entry(newparentdirloc, newname, tmpmode)
    ...
    def import_save_file(self, sf, ignore_existing, dirname = None):
        ...
        if dirname == None:
            dir_str = dir_ent[8].decode('utf-8', 'ignore') if isinstance(dir_ent[8], bytes) else str(dir_ent[8])
            dirname = "/" + dir_str
        ...
        try:
            ...
            for i in range(dir_ent[2]):
                ...
                e8_str = ent[8].decode('utf-8', 'ignore') if isinstance(ent[8], bytes) else str(ent[8])
                (dirloc, ent) = self.create_dir_entry(dir_dirloc, ent[8], mode)
                f = self.file(dirloc, ent[4], ent[2], "wb", dirname + e8_str)
                try:
                    f.write(data)
                finally:
                    f.close()
        ...
        try:
            for i in range(2, dirent[2]):
                ...
                if not mode_is_file(ent[0]):
                    d8_str = dirent[8].decode('utf-8', 'ignore') if isinstance(dirent[8], bytes) else str(dirent[8])
                    e8_str = ent[8].decode('utf-8', 'ignore') if isinstance(ent[8], bytes) else str(ent[8])
                    print("warning: %s/%s is not a file, ignored." % (d8_str, e8_str))
                    continue
    ...
    def _remove_dir(self, dirloc, ent, dirname):
        first_cluster = ent[4]
        length = ent[2]
        dir = self._directory(dirloc, first_cluster, length, "rb", dirname)
        try:
        ...
        for (i, ent) in ents[2:]:
            ...
            e8_str = ent[8].decode('utf-8', 'ignore') if isinstance(ent[8], bytes) else str(ent[8])
            if mode & DF_DIR:
                self._remove_dir((first_cluster, i), ent, dirname + e8_str + "/")
            else:
                self.delete_dirloc((first_cluster, i), False, dirname + e8_str)
    ...
    def _check_dir(self, fat, dirloc, dirname, ent):
        ...
        d8_str = dot_ent[8].decode('utf-8', 'ignore') if isinstance(dot_ent[8], bytes) else str(dot_ent[8])
        if d8_str != ".":
        ...
        dd8_str = dir[1][8].decode('utf-8', 'ignore') if isinstance(dir[1][8], bytes) else str(dir[1][8])
        if dd8_str != "..":
            print("bad directory:", (dirname + ': missing ".." entry'))
            ret = False
        ...
        for i in range(2, length):
            ...
            if mode & DF_DIR:
                ...
                if why != None:
                    print("bad file:", (dirname + e8_str + ":"), why)
                    ret = False
    ...
    def check(self):
        ...
        for i in range(self.allocatable_cluster_end):
            ...
            if (a & PS2MC_FAT_ALLOCATED_BIT) and not fat[i]:
                ...
                lost_clusters += 1
        if lost_clusters > 0:
            print("\nfound", lost_clusters, "lost clusters")
            ret = False
        ...
        try:
            ...
            for ent in dir:
                ...
                if ((ent[0] & DF_EXISTS) == 0 or (ent[0] & DF_DIR) == 0):
                    continue
                if name == "." or name == ".." or name == b"." or name == b"..":
                    if pattern != name:
                        continue
                elif not fnmatch.fnmatchcase(name, pattern):
                    continue
                ret += _glob(dirname + name + "/", components, is_dir)
        finally:
            dir.close()
        return ret
    ...
    def dir_size(self, dirname):
        dir = self.dir_open(dirname)
        try:
            length = round_up(len(dir) * PS2MC_DIRENT_LENGTH, self.cluster_size)
            for ent in dir:
                if mode_is_file(ent[0]):
                    length += round_up(ent[2], self.cluster_size)
                elif (mode_is_dir(ent[0]) and ent[8] not in [".", "..", b".", b".."]):
                    name_str = ent[8] if isinstance(ent[8], str) else ent[8].decode('utf-8')
                    length += self.dir_size(dirname + "/" + name_str)
        finally:
            dir.close()
        return length
