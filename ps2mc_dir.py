...
def zero_terminate(s):
    """Truncate a string or bytes at the first NUL ('\0') character, if any."""
...
def unpack_dirent(s):
    ...
    # Decode the raw byte array into a Python 3 string
    name_bytes = zero_terminate(ent[8])
    ent[8] = name_bytes.decode('utf-8', 'ignore') if isinstance(name_bytes, bytes) else name_bytes
    return ent
...
def pack_dirent(ent):
    ...
    # Re-encode the string back into bytes before writing to the image
    if isinstance(ent[8], str):
