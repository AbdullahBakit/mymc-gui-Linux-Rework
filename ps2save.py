...
def shift_jis_conv(src, encoding = None):
    ...
    if encoding == "shift_jis":
        return src.decode("shift_jis", "replace") if isinstance(src, bytes) else src
    u = src.decode("shift_jis", "replace") if isinstance(src, bytes) else src
    if encoding == "unicode" or encoding == "utf-8":
        return u
    a = []
    for uc in u:
        try:
    ...
    # Return a standard Python 3 string, never raw bytes
    return "".join(a)
...
class ps2_save_file(object):
    ...
    def get_icon_sys(self):
        for i in range(self.dirent[2]):
            ...
            # The ent[8] from struct unpack is typically bytes
            name = ent[8].decode('utf-8', 'ignore') if isinstance(ent[8], bytes) else ent[8]
            if name == "icon.sys" and len(data) >= 964:
    ...
    def _load_max_drive_2(self):
        ...
        if lzari == None:
            raise error("The lzari module is needed to decompress MAX Drive saves.")
        
        dir_name = self.dirent[8]
        dir_str = dir_name.decode('utf-8', 'replace') if isinstance(dir_name, bytes) else dir_name
        progress_str = "decompressing " + dir_str + ": "
        
        s = lzari.decode(s, length, progress_str)
    ...
    def save_max_drive(self, f):
        ...
        if icon_sys != None:
            ...
            if len(title[0]) > 0 and title[0][-1:] != b' ':
                iconsysname = title[0] + b" " + title[1].strip()
            else:
        ...
        for i in range(dirent[2]):
            ...
            s += struct.pack("<L32s", ent[2], ent_name)
            s += data
        ...
        dir_name = dirent[8]
        dir_str = dir_name.decode('utf-8', 'replace') if isinstance(dir_name, bytes) else dir_name
        progress_str = "compressing " + dir_str + ": "
        
        compressed = lzari.encode(s, progress_str)
        
        dir_name_bytes = dirent[8] if isinstance(dirent[8], bytes) else dirent[8].encode('utf-8')
        
        hdr = struct.pack("<12sL32s32sLLL", PS2SAVE_MAX_MAGIC,
                  0, dir_name_bytes, iconsysname,
        ...
        f.write(struct.pack("<12sL32s32sLLL", PS2SAVE_MAX_MAGIC,
                    crc & 0xFFFFFFFF, dir_name_bytes, iconsysname,
                    len(compressed) + 4, dirent[2], length))
        f.write(compressed)
        f.flush()
...
def detect_file_type(f):
    ...
    dir_name_str = dirent[8].decode('utf-8', 'replace') if isinstance(dirent[8], bytes) else dirent[8]
    dot_name_str = dotent[8].decode('utf-8', 'replace') if isinstance(dotent[8], bytes) else dotent[8]
    dotdot_name_str = dotdotent[8].decode('utf-8', 'replace') if isinstance(dotdotent[8], bytes) else dotdotent[8]
    
    if (mode_is_dir(dirent[0]) and mode_is_dir(dotent[0])
        and mode_is_dir(dotdotent[0]) and dirent[2] >= 2
        and dot_name_str == "." and dotdot_name_str == ".."):
        return "psu"
    return None
...
#
_bad_filename_chars = "".join(chr(i) for i in range(32)) + "".join(chr(i) for i in range(127, 256))
...
def make_longname(dirname, sf):
    ...
    title = ""
    if icon_sys != None:
        title_tuple = icon_sys_title(icon_sys, "unicode")
        title = title_tuple[0] + " " + title_tuple[1]
        title = " ".join(title.split())
    crc = binascii.crc32(b"")
    for (ent, data) in sf:
        crc = binascii.crc32(data, crc)
        
    dirname_str = dirname.decode('utf-8', 'replace') if isinstance(dirname, bytes) else dirname

    if len(dirname_str) >= 12 and (dirname_str[0:2] in ("BA", "BJ", "BE", "BK")):
        if dirname_str[2:6] == "DATA":
            title = ""
        else:
            dirname_str = dirname_str[2:12]

    return fix_filename("%s %s (%08X)" % (dirname_str, title, crc & 0xFFFFFFFF))